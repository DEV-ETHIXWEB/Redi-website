...
const CRITICAL_PATTERNS = [
  {
    re: /String\.fromCharCode\(\s*(?:127|0x7f)\s*\)/i,
    why: 'fromCharCode(127) obfuscation (incident signature)',
  },
  {
    re: /fromCharCode\((?:[^)]*,)?\s*(?:127|0x7f)\s*(?:,[^)]*)?\)/i,
    why: 'fromCharCode with DEL byte',
  },
  {
    re: /global(?:\.[ioe]|\[['"][ioe]['"]\])\s*=\s*['"]7-[vw]?\d+/i,
    why: "7-v campaign marker (global.o='7-v####' injector tag)",
  },
  ...
  { re: /_\$_\d{3,}\s*=/, why: '_$_#### deobfuscator variable (incident payload)' },
  {
    ...
    why: 'base64-decode piped to shell (persistence)',
  },
  { re: /\b(?:curl|wget)\b[^\n|]*\|\s*(?:sh|bash|zsh)\b/i, why: 'remote script piped to shell' },
  {
    re: /eval\s*\(\s*(?:atob|Buffer\.from|require\(['"]zlib['"]\))/i,
    why: 'eval of decoded/base64 payload',
  },
  {
    re: /child_process[\s\S]{0,80}?(?:atob|Buffer\.from\([^)]*base64)/i,
    why: 'child_process executing decoded payload',
  },
  ...
  {
    re: /printf\s+\S+\s*\|\s*tr\s+-d\b/i,
    why: 'runtime-built decoder name (printf|tr -d obfuscation)',
  },
  // Encoded blob decoded then piped to a shell:  echo <base64> | q | sh
  {
    re: /echo\s+[A-Za-z0-9+/]{40,}={0,2}\s*\|[^\n]{0,60}\|\s*(?:sh|bash|zsh)\b/i,
    why: 'encoded blob decoded and piped to shell',
  },
  // Hex-encoded blob echoed into a pipe (xxd variant)
  {
    re: /echo\s+(?:[0-9a-fA-F]{2}){40,}\s*\|/i,
    why: 'hex-encoded blob piped to decoder (xxd variant)',
  },
  // Campaign beacon parameters (curl -d "p=<stage>")
  {
    re: /-d\s*["']p=(?:xcode_phase|android_gradle|Daemon|Terminal|SRC)\b/i,
    why: 'injector C2 beacon parameter (p=<stage>)',
  },
  // Gradle Groovy shell-exec: ["sh","-c", ...].execute()
  {
    re: /\[\s*["']sh["']\s*,\s*["']-c["'][\s\S]{0,400}?\]\s*\.execute\s*\(/i,
    why: 'Gradle shell-exec of inline command (.execute())',
  },
  // Backgrounded decode-and-run subshell wrapper used by this campaign
  {
    re: /\(\(\s*[A-Za-z_]\w*\s*\(\)\s*\{[\s\S]{0,160}?\}\s*;\s*echo[\s\S]{0,600}?\|\s*[A-Za-z_]\w*\s*\|\s*sh\b/i,
    why: 'decode-and-exec subshell (build-phase injector)',
  },
];
...
function scanFile(file) {
  ...
  try {
    if (path.resolve(file) === __filename) return;
  } catch {
    /* ignore */
  }
  ...
  if (isConfig) {
    ...
    if (buf.length > CONFIG_MAX_BYTES) {
      warnings.push(
        `${rel}: config file is ${buf.length}B (unusually large — verify it wasn't tampered)`,
      );
    }
  ...
  if (/(^|\/)package\.json$/.test(rel)) {
    try {
      ...
      for (const k of ['preinstall', 'install', 'postinstall']) {
        if (scripts[k])
          warnings.push(`${rel}: has "${k}" script → "${scripts[k]}" (review it is intentional)`);
      }
    } catch {
      /* ignore parse errors */
    }
